...
def get_or_create_soul(name: str) -> Tuple[int, bool, str, int]:
    """Get existing soul by name or create new one. Returns (soul_id, is_new, placement, points)"""
    conn = get_db()
    cur = conn.cursor()
    
    # Check if soul exists
    cur.execute("SELECT id, placement, total_points FROM souls WHERE name = %s", (name,))
    result = cur.fetchone()
    
    if result:
        soul_id, placement, points = result
        # Update last active timestamp
        cur.execute("UPDATE souls SET last_active = CURRENT_TIMESTAMP WHERE id = %s", (soul_id,))
        conn.commit()
        cur.close()
        conn.close()
        return soul_id, False, placement, points
    
    # Soul doesn't exist, need to create
    cur.close()
    conn.close()
    return None, True, None, None

def create_soul(name: str, life_story: str) -> Tuple[int, str, int]:
    """Create new soul and calculate placement. Returns (soul_id, placement, points)"""
    conn = get_db()
    cur = conn.cursor()
    ...
    return soul_id, placement, total_points
...
def main():
    ...
    # Check if soul exists
    soul_id, is_new, placement, points = get_or_create_soul(name)
    
    if not is_new:
        # Returning soul
        print(f"\n👋 Welcome back, {name}!")
        print(f"📍 You are in the {placement.upper()} PLACE")
        print(f"📊 Your points: {points}")
        
        # Show conversation count
        conn = get_db()
        cur = conn.cursor()
        cur.execute("SELECT COUNT(*) FROM conversations WHERE soul_id = %s AND role = 'user'", (soul_id,))
        msg_count = cur.fetchone()[0]
        cur.close()
        conn.close()
        
        print(f"💬 Previous conversations: {msg_count} messages")
        print("\n🔄 Continuing your eternal journey...\n")
    else:
        # New soul
        ...
        if not life_story.strip():
            print("❌ Life story cannot be empty!")
            return
        
        print("\n⏳ Calculating your eternal placement...")
        soul_id, placement, points = create_soul(name, life_story)
